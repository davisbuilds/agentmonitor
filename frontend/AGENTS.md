...
- `src/lib/components/trace-quality/`: Quality sub-view with an Explorer | Dashboards panel toggle. Explorer: `TraceQualityPage` (list + inspector), `TraceTree` (recursive observation tree), `TraceCoverageBadge`, `ScoreEditor` (human-only local review scores), `TraceDrillInLink` (session drill-in from Usage/Analytics/Live/Sessions/Search). Dashboards: `QualityFindings` (finding cards + `kind`/`severity` filters + Inspect-into-explorer), `PromptRollups` (prompt-version table), `ScoreTrends` (score summary + rollups by dimension). Backed by `stores/trace-quality.svelte.ts`; `?session=`/`?trace=` ride the analytics hash; dashboards load lazily on first open.
