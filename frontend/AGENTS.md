...
- `src/lib/components/trace-quality/`: Quality sub-view (local trace-quality explorer). `TraceQualityPage` (list + inspector), `TraceTree` (recursive observation tree), `TraceCoverageBadge`, `ScoreEditor` (local review scores), `TraceDrillInLink` (session drill-in from Usage/Analytics/Live/Sessions/Search). Backed by `stores/trace-quality.svelte.ts`; `?session=`/`?trace=` ride the analytics hash.
