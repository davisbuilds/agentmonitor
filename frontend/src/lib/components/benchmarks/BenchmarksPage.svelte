<script lang="ts">
  ...
  const caveats = $derived.by(() => {
    ...
    for (const a of d.arms.filter(a => a.ranking_eligible === false)) {
            ...
            {#snippet cell(arm, column)}
              ...
              {:else if column.key === 'flags'}
                <span class="flex flex-wrap gap-1">
                  ...
                  {#if arm.ranking_eligible === false}<Badge tone="warn" title={arm.ranking_exclusion_reason ?? 'usage excluded from ranking'}>ineligible</Badge>{/if}
                </span>
              {/if}
            {/snippet}
          </DataTable>
