import { applyBroadcastStats, setServerBuild, incrementEvent, addEvent, handleSessionUpdate, handleEventForSession, setConnectionStatus, setQuotaMonitor, signalReconnect } from './monitor.svelte';
...
function dispatch(msg: { type: string; payload: unknown }): void {
  switch (msg.type) {
    ...
    case 'stats': {
      ...
      setServerBuild(statsPayload.server_build);
