...
function deriveTimestamp(row: EventRow): string {
  // events.created_at is generated by SQLite datetime('now'), which is UTC
  // but omits the offset. Do not let consumers reinterpret it as host-local time.
  // Restrict this conversion to the known database format, never client input.
  const fallback = /^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$/.test(row.created_at)
    ? `${row.created_at.replace(' ', 'T')}Z`
    : row.created_at;
  return row.client_timestamp ?? fallback;
}
