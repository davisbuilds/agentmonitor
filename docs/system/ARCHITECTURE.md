...
- Invocation mode: `claude-code.ts` reads each line's `entrypoint`/`promptSource` and `codex.ts` reads `session_meta.originator` (see `src/util/invocation-mode.ts`) to derive an `interactive`/`headless` mode. It rides events as `NormalizedIngestEvent.mode`, is persisted into `sessions.metadata.mode` (by `upsertSession` on fresh events, and by `setSessionMode` once per session per imported file so it also backfills when every event is a duplicate), and is surfaced as a derived `mode` column by the monitor session queries. Because the live Claude Monitor stream is hook-sourced (hooks carry no `entrypoint`) and Codex is OTEL-sourced, the mode is filled by the periodic auto-import (`runImport`, 5s after boot then on interval) rather than the live event path — a just-started session may be unmarked until the next import tick, at which point the server broadcasts a `session_update {type:'auto_import'}` that the Monitor uses to refetch (so the pill appears without a manual reload). A forced re-import (`amon import --force`) backfills mode for sessions imported before this feature existed. Antigravity has no equivalent signal and is left unmarked.
