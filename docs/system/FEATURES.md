...
- Skill analytics now include explicit Claude `Skill` tool calls plus inferred Codex skill reads from `.../SKILL.md` commands captured through OTEL or Codex JSONL fallback. Both the legacy `exec_command` and newer `exec` Codex tool names are recognized.
