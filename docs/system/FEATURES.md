...
- Batch-imported benchmark events (`source='benchmark'`, from `amon import benchmark`) are **excluded** from every usage/analytics aggregate by default so an openbench bake-off does not skew real-activity numbers. Pass `include_benchmark` to opt them in; the exclusion is applied once at the shared usage-filter seam, so all usage panels honor it uniformly.
