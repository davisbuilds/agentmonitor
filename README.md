...
- Exposes event-derived historical usage under `/api/v2/usage/*`, including summary totals, daily series, project/model/tier/agent attribution, cache economics, prior-period comparison, read-only budget reports, and top sessions with coverage metadata.
...
- Usage responses include coverage metadata so the UI can distinguish usage-bearing events from matching events that carry no cost or token data. Usage endpoints accept `date_from`, `date_to`, `project`, `agent`, `model`, `provider`, and `tier` filters. Stored `cost_usd` remains the source of truth for event cost; cache hit rate, estimated cache savings, prior-period deltas, and read-only budget states are derived at query time.
