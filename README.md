...
- Usage responses include coverage metadata so the UI can distinguish usage-bearing events from matching events that carry no cost or token data. Usage endpoints accept `date_from`, `date_to`, `project`, `agent`, `model`, `provider`, and `tier` filters. Stored `cost_usd` remains the source of truth for event cost; cache hit rate, estimated cache savings, prior-period deltas, read-only budget states, and human-reviewed tier feedback are derived at query time.
